...
class BaseService
{
    ...
    public static function anti_injection($input){
        ...
        $filter = self::mysql_escape(stripslashes(strip_tags(htmlspecialchars($clean, ENT_QUOTES ))));
    ...
    public static function sql_select_all($type, $where_clause, $group_by_clause, $order_by_clause){    

        $table_name = $type; 
        $select_field = "*";

        $sql = "select $select_field from $table_name a";
        
        if($where_clause && !empty($where_clause)){
            $sql .= " WHERE $where_clause";
        }
        
        if($group_by_clause && !empty($group_by_clause)){
            $sql .= " GROUP BY $group_by_clause";
        }

        if($order_by_clause && !empty($order_by_clause)){
