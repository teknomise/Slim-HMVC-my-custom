;(function () {
	...
	var Slides = function() {
		...
		var dat;
		var destinationPrefixName = [1,2,3,4,5];
		var img_first = $('body').find('.img-slides-1').data("image");

		$('body').find('#fh5co-header').css('background-image', 'url(' + img_first + ')').fadeIn("slow");
		...
		$.each(destinationPrefixName, function( index, value ) {
			...
			$(".active-" + value).on('click', function(){
			  ...
			  $.each(destinationPrefixName, function(index, _value) {
				...
				dat = $('body').find('.img-slides-'+value);

			  });
			  
				var data_image = dat.data("image");
				var elem = $('body').find('#fh5co-header');

			    $(elem).delay( 150 ).fadeTo(100, 0.5, function()
			    {
